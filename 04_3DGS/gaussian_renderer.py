...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        # Apply world->camera rotation to each covariance: Sigma_cam = R * Sigma_world * R^T.
    ...
    def compute_gaussian_values(
        ...
        det = torch.det(covs2D).clamp(min=1e-8)  # (N,)
        inv_covs2D = torch.inverse(covs2D)  # (N, 2, 2)

        mahal = torch.einsum('nhwi,nij,nhwj->nhw', dx, inv_covs2D, dx)  # (N, H, W)
        norm = 1.0 / (2.0 * np.pi * torch.sqrt(det)).view(N, 1, 1)  # (N, 1, 1)
        gaussian = norm * torch.exp(-0.5 * mahal)  # (N, H, W)
