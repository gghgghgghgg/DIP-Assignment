...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    lap_fg = torch.nn.functional.conv2d(foreground_img, lap_kernel, padding=1, groups=3)
    lap_blended = torch.nn.functional.conv2d(blended_img, lap_kernel, padding=1, groups=3)

    fg_mask_3 = (foreground_mask > 0.5).expand(-1, 3, -1, -1)
    bg_mask_3 = (background_mask > 0.5).expand(-1, 3, -1, -1)

    fg_vals = lap_fg[fg_mask_3]
    bg_vals = lap_blended[bg_mask_3]

    n = min(fg_vals.numel(), bg_vals.numel())
    if n == 0:
        # 只为防止 backward 崩溃；不影响框架其它逻辑
        return torch.zeros((), device=foreground_img.device, requires_grad=True)

    loss = torch.mean(torch.abs(fg_vals[:n] - bg_vals[:n]))
    return loss
